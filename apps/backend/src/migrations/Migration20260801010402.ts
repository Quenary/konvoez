...
export class Migration20260801010402 extends Migration {
  override up(): void | Promise<void> {
    ...
    this.addSql(
      `create table \`users\` (\`id\` integer not null primary key autoincrement, \`created_at\` datetime not null, \`updated_at\` datetime null, \`username\` text not null, \`password\` text not null, \`fullname\` text not null, \`email\` text not null, \`role\` text check (\`role\` in ('OWNER', 'ADMIN', 'MEMBER')) not null default 'MEMBER', \`avatar\` text null);`,
    );
    this.addSql(
    ...
    this.addSql(
      `create index \`users_fullname_index\` on \`users\` (\`fullname\`);`,
    );
