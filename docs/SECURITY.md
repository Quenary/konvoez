...
- **Web Push notifications**:
  - Optional browser push for personal (direct) messages.
  - Delivery uses the Web Push protocol: the payload is encrypted in transit with the browser subscription keys (`p256dh` / `auth`) so a push service (FCM, Mozilla Autopush, etc.) cannot read it.
  - The application still places a plaintext message preview inside that envelope so the operating system can render the notification. This is an intentional trade-off: the Konvoez server and the recipient device can see the preview, while message bodies remain encrypted at rest in the database.
  - VAPID keys are stored on disk next to sqlite and uploaded files. Autogenerated keys are encrypted with `MASTER_KEY` using the same HKDF-SHA256 derivation as chat encryption (with a separate key-info label). Operators may mount their own `vapid.json`, including an unencrypted JSON key pair.
- **Voice communication**:
